...
pub async fn led_handler(State(state): State<Arc<AppState>>, Json(body): Json<LedRequest>) -> Result<Json<SuccessResponse>> {
    let sessions = state.sessions.lock().await;
    let session = sessions
        .get(&body.session_id)
        .ok_or_else(|| WsError::SessionNotFound(body.session_id.clone()))?;
    let color = LedColor::from_str(&body.color).map_err(|e| WsError::InvalidColor(e.to_string()))?;
