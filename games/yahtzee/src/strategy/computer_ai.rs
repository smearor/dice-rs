...
mod tests {
    ...
    #[test]
    fn decide_yahtzee_enters_score() {
        ...
        // Yahtzee (50 points) is so good, AI should enter it immediately
        match decision {
            AiDecision::EnterScore { category, score } => {
                assert_eq!(category, ScoreCategory::Yahtzee);
            ...
            AiDecision::CrossOut { .. } => panic!("should not cross out with Yahtzee"),
        }
    }
    ...
    fn decide_rolls_exhausted_enters_best() {
        ...
        match decision {
            AiDecision::EnterScore { category, score } => {
                // Yahtzee (50) is the highest-scoring category for five 3s
                assert_eq!(category, ScoreCategory::Yahtzee);
                assert_eq!(score, Score::new(50));
            }
    ...
    fn hint_best_category() {
        ...
        assert_eq!(hint.category(), ScoreCategory::Yahtzee);
