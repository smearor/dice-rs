...
impl Probability {
    ...
    pub fn new(value: f64) -> Result<Self> {
        if !(0.0..=1.0).contains(&value) {
            return Err(YahtzeeError::InvalidProbability(value));
        }
        Ok(Self(value))
    }
    ...
    pub fn from_fraction(numerator: u32, denominator: u32) -> Result<Self> {
        if denominator == 0 {
            return Err(YahtzeeError::InvalidProbability(f64::NAN));
        }
...
/// Calculate the probability of rolling a Yahtzee (all 5 dice same value)
/// when rolling `dice_count` dice.
pub fn probability_of_yahtzee(dice_count: usize) -> Result<Probability> {
...
mod tests {
    ...
    #[test]
    fn probability_of_yahtzee_five_dice() {
        let p = probability_of_yahtzee(5).unwrap();
        // There are 6 Yahtzee outcomes out of 7776
