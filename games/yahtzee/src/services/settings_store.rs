...
impl SettingsStore {
    ...
    ///
    /// Uses `XDG_CONFIG_HOME/yahtzee/settings.json` on Linux.
    pub fn default_path() -> Result<Self> {
        let config_dir = dirs::config_dir().ok_or(YahtzeeError::SettingsIoError("no config directory".to_string()))?;
        let settings_dir = config_dir.join("yahtzee");
    ...
    pub async fn load(&self) -> Result<Option<GameSettings>> {
        ...
        file.read_to_string(&mut contents)
            .await
            .map_err(|e| YahtzeeError::SettingsIoError(e.to_string()))?;
        let settings: GameSettings = serde_json::from_str(&contents).map_err(|e| YahtzeeError::SettingsParseError(e.to_string()))?;
        Ok(Some(settings))
    }
    ...
    pub async fn save(&self, settings: &GameSettings) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(settings).map_err(|e| YahtzeeError::SettingsParseError(e.to_string()))?;
...
mod tests {
    ...
    async fn save_and_load_roundtrip() {
        ...
        let path = temp_dir.join("yahtzee_test_settings.json");
    ...
    async fn load_nonexistent_returns_none() {
        let path = std::env::temp_dir().join("yahtzee_nonexistent_settings.json");
        let _ = std::fs::remove_file(&path);
        let store = SettingsStore::new(&path);
        let loaded = store.load().await.unwrap();
