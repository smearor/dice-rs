...
/// score, or crossed out. The scorecard also tracks Yahtzee bonus
/// points awarded for additional Yahtzees rolled after the Yahtzee
...
pub struct Scorecard {
    ...
    /// Bonus points for additional Yahtzees (100 per additional Yahtzee).
    yahtzee_bonus: Score,
}
...
impl Scorecard {
    ...
    pub fn enter(&mut self, category: ScoreCategory, score: Score) -> Result<()> {
        ...
        if self.entries[idx].is_used() {
            return Err(YahtzeeError::CategoryAlreadyFilled(category));
        }
    ...
    /// Yahtzee bonus points (100 per additional Yahtzee).
    pub fn yahtzee_bonus(&self) -> Score {
        self.yahtzee_bonus
    }

    /// Add a Yahtzee bonus (100 points). Called when a second Yahtzee
    /// is rolled and the Yahtzee category is already filled with 50.
    pub fn add_yahtzee_bonus(&mut self) {
        self.yahtzee_bonus += Score::new(100);
    }

    /// Grand total including upper subtotal, upper bonus, lower subtotal,
    /// and Yahtzee bonus.
    pub fn grand_total(&self) -> Score {
        self.upper_subtotal() + self.upper_bonus() + self.lower_subtotal() + self.yahtzee_bonus()
    }
...
mod tests {
    ...
    fn cross_out() {
        let mut sc = Scorecard::new();
        sc.cross_out(ScoreCategory::Yahtzee).unwrap();
        assert!(sc.is_filled(ScoreCategory::Yahtzee));
        assert_eq!(sc.score(ScoreCategory::Yahtzee), Score::ZERO);
    }

    #[test]
    fn cross_out_already_filled_fails() {
        let mut sc = Scorecard::new();
        sc.enter(ScoreCategory::Yahtzee, Score::new(50)).unwrap();
        assert!(sc.cross_out(ScoreCategory::Yahtzee).is_err());
    }
    ...
    fn yahtzee_bonus() {
        ...
        sc.add_yahtzee_bonus();
        sc.add_yahtzee_bonus();
        assert_eq!(sc.yahtzee_bonus(), Score::new(200));
    }
}
