...
impl PlayerSetup {
    ...
    pub fn new(configs: Vec<PlayerConfig>) -> Result<Self> {
        if configs.is_empty() {
            return Err(YatzyError::NotEnoughPlayers(0));
        }
        if configs.len() > PlayerColor::DEFAULTS.len() {
            return Err(YatzyError::TooManyPlayers(configs.len()));
        }
        ...
        for i in 0..configs.len() {
            for j in (i + 1)..configs.len() {
                if configs[i].name == configs[j].name {
                    return Err(YatzyError::DuplicateName(configs[i].name.as_str().to_string()));
                }
                if configs[i].color == configs[j].color {
                    return Err(YatzyError::DuplicateColor(configs[i].color.to_string()));
                }
            }
        }
