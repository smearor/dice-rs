...
impl HighscoreStore {
    ...
    ///
    /// Uses `XDG_DATA_HOME/yatzy/highscore.json` on Linux.
    pub fn default_path() -> Result<Self> {
        let data_dir = dirs::data_dir().ok_or(YatzyError::SettingsIoError("no data directory".to_string()))?;
        let highscore_dir = data_dir.join("yatzy");
    ...
    pub async fn load(&self) -> Result<Option<HighscoreList>> {
        ...
        let list: HighscoreList = serde_json::from_str(&contents).map_err(|e| YatzyError::SettingsParseError(e.to_string()))?;
        Ok(Some(list))
    }
    ...
    pub async fn save(&self, list: &HighscoreList) -> Result<()> {
        ...
        let json = serde_json::to_string_pretty(list).map_err(|e| YatzyError::SettingsParseError(e.to_string()))?;
...
mod tests {
    ...
    async fn save_and_load_roundtrip() {
        ...
        let path = temp_dir.join("yatzy_test_highscore.json");
    ...
    async fn load_nonexistent_returns_none() {
        let path = std::env::temp_dir().join("yatzy_nonexistent_highscore.json");
        let _ = std::fs::remove_file(&path);
        let store = HighscoreStore::new(&path);
        let loaded = store.load().await.unwrap();
