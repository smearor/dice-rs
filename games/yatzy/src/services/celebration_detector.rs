...
impl CelebrationDetector {
    ...
    pub fn effect_for(result: RollResult) -> Option<LedEffect> {
        match result {
            RollResult::Yatzy => Some(LedEffect::yatzy()),
...
mod tests {
    ...
    fn classify_yatzy() {
        ...
        assert_eq!(CelebrationDetector::classify(&d), RollResult::Yatzy);
    }
    ...
    #[test]
    fn effect_for_yatzy() {
        let effect = CelebrationDetector::effect_for(RollResult::Yatzy);
        assert!(effect.is_some());
        assert_eq!(effect.unwrap(), LedEffect::yatzy());
    }
