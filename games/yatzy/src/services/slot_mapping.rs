...
impl SlotMapping {
    ...
    pub fn assign(&self, slot: DiceSlot, dice: Dice) -> crate::error::Result<()> {
        ...
        {
            let mut slots = self.slots.lock().map_err(|_| crate::error::YatzyError::LockPoisoned)?;
            slots[slot.get() as usize] = Some(dice);
        }
        {
            let mut map = self.name_to_slot.lock().map_err(|_| crate::error::YatzyError::LockPoisoned)?;
