...
impl LedControls {
    ...
    fn connect_signals(&self) {
        ...
        self.color_button1.connect_color_set(move |button| {
            let rgba = button.rgba();
            let color = LedColor::new((rgba.red() * 255.0) as u8, (rgba.green() * 255.0) as u8, (rgba.blue() * 255.0) as u8);
            dice_service.set_led1(color);
        });

        // LED 2 color picker - set LED 2 only (LED 1 stays off).
        let dice_service = self.dice_service.clone();
        self.color_button2.connect_color_set(move |button| {
            let rgba = button.rgba();
            let color = LedColor::new((rgba.red() * 255.0) as u8, (rgba.green() * 255.0) as u8, (rgba.blue() * 255.0) as u8);
            dice_service.set_led2(color);
        });
        ...
        self.set_button.connect_clicked(clone!(
            ...
            move |_| {
                ...
                dice_service.set_leds(color1, color2);
            }
        ));
        ...
        self.pulse_button.connect_clicked(clone!(
            ...
            move |_| {
                ...
                dice_service.pulse_leds(5, 10, 10, color, blink_mode, leds);
            }
        ));

        // Off button - turn both LEDs off.
        let dice_service = self.dice_service.clone();
        self.off_button.connect_clicked(move |_| {
            dice_service.turn_off_leds();
        });
    }
}

impl WidgetContainer for LedControls {
